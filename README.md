...
| `CEZ_UNITS=1` | Turn on **Units** — the hierarchical army of runs (Caesar → Legate → Centurion): the Missions view appears, and a running unit can delegate by ending a turn with `CEZ:SPAWN`, which creates child runs below it (any rank below the spawner; implementers, reviewers, researchers or planners by `kind`) — each in its OWN worktree, forked off the parent's branch, which the parent merges back once it has validated the work, optionally after a separate review unit approves it. Units talk through the mission directory (`.ai/cezar/missions/<id>/`: the brief, each unit's order/notes/report, an inbox per unit) and cezar wakes a parked recipient when a file lands. A mission can set its own `parallel` limit (its runs are then exempt from the workspace cap) and `maxChildren` per commander (default 4). Off by default, and only the exact value `1` enables it — without it the markers are not parsed, no role prompt is composed, the missions/units endpoints answer `409`, and the nav item is absent. Read at boot, so restart after changing it. This is the widest cost-widening flag here — one turn can start four more agents — so set a mission budget in the composer; per-repo role prompts under `.ai/cezar/units/` are retained, so unsetting it and restarting restores the feature without migration or data loss. |
