...
| `CEZ_DISABLE_REPO_LOCK=1` | **Dangerous escape hatch:** allow runs explicitly using the current checkout (`worktree=false`) to execute concurrently without Cezar’s repository-root lease. Agents can overwrite each other’s files or Git state; isolated worktree runs are unaffected. |
