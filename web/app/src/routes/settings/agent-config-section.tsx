...
export function AgentConfigSection() {
  ...
  if (listing.isPending) {
    return (
      <p data-slot="agent-config-loading" className="p-4 text-sm text-soft-foreground md:p-6">
    ...
    <div data-slot="agent-config" className="flex flex-col gap-4 p-4 md:p-6">
      {!listing.editable && (
        <div
          ...
          className="rounded-md border border-border bg-muted/40 px-3 py-2 text-sm text-soft-foreground"
        >
        ...
        {AGENT_DESCRIPTORS.map((d) => (
          <button
            ...
            className={cn(
              ...
              d.id === agent.id ? 'bg-background font-semibold shadow-xs' : 'hover:bg-muted/60',
            )}
          >
            {d.label}
            {!installed.includes(d.id) && (
              <Badge variant="outline" className="text-2xs text-soft-foreground">
                not installed
              </Badge>
            )}
          </button>
      ...
      {agent.note && (
        <p data-slot="agent-config-agent-note" className="text-xs text-soft-foreground">
          {agent.note}
        </p>
      )}
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,20rem)_minmax(0,1fr)]">
        ...
        <div data-slot="agent-config-editor-pane">
          {selected ? (
            ...
            <div className="flex h-full min-h-40 items-center justify-center rounded-md border border-dashed border-border text-sm text-soft-foreground">
      ...
      {agent.groups.map((g) => {
        ...
        return (
          <section key={g.id} data-slot="agent-config-group" data-group={g.id} data-agent={agent.id}>
            <p className="mb-1 label-caps">
              {g.label}
            </p>
            {g.note && <p className="mb-2 text-xs text-soft-foreground">{g.note}</p>}
            <ul className="flex flex-col gap-1">
              {files.map((file) => (
                <li key={file.id}>
                  ...
                  >
                    <span className="min-w-0 flex-1 truncate font-mono text-xs">{file.label}</span>
                    {file.seeded && (
                      <Badge variant="outline" className="shrink-0 text-2xs">
                        seeded
                      </Badge>
                    )}
                    {!file.exists && <span className="shrink-0 text-2xs text-soft-foreground">absent</span>}
                  </button>
                </li>
              ))}
            </ul>
...
function UserMcpBlock({ userMcp }: { userMcp: NonNullable<AgentConfigListing['userMcp']> }) {
  return (
    <div data-slot="agent-config-user-mcp" className="mt-3">
      <h4 className="mb-1 text-xs font-semibold">User &amp; local scopes</h4>
      <p className="mb-2 text-xs text-soft-foreground">
      ...
      {userMcp.readable ? (
        userMcp.servers.length > 0 ? (
          ...
          <p className="text-xs text-soft-foreground">No user-scoped MCP servers.</p>
        )
      ) : (
        <p className="text-xs text-soft-foreground">Could not read the file.</p>
      )}
    </div>
  )
}
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex flex-wrap items-center gap-2">
        <span className="font-mono text-sm">{file.label}</span>
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="text-xs text-soft-foreground underline hover:text-foreground"
        >
          docs
        </a>
      </div>

      <p data-slot="agent-config-precedence" className="text-xs text-soft-foreground">
        {file.precedence}
      </p>
      <p data-slot="agent-config-effect" className="text-xs text-foreground/80">
        {effectLabel(file)}
        {file.hotReload ? ` ${file.hotReload}` : ''}
      </p>

      {fileQuery.isPending ? (
        <p className="text-sm text-soft-foreground">Loading file…</p>
      ) : fileQuery.isError ? (
      ...
      {formatError && (
        <p data-slot="agent-config-format-error" className="text-xs text-danger">
          {formatError}
        </p>
      )}
      {conflict && (
        <div
          ...
          className="flex items-center justify-between gap-2 rounded-md border border-danger/40 bg-danger/10 px-3 py-2 text-xs"
        >
      ...
      {canWrite && (
        <div className="flex items-center gap-2">
          ...
          {dirty && <span className="text-xs text-soft-foreground">Unsaved changes</span>}
        </div>
      )}
    </div>
  )
