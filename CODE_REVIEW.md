...
- Treat a spawned Claude Code agent as having full shell access in its worktree: the zero-config default allowlist includes unrestricted `Bash`, and unapproved tools are denied without prompting (`--permission-mode dontAsk`). Settings → Permissions `Auto` is the opt-in skip-all (`--dangerously-skip-permissions`); `CEZ_APPROVAL_GATE=1` still forces `acceptEdits`. Codex/OpenCode ignore `allowedTools` (Codex: own sandbox, approvals off, network on; OpenCode: auto-approved unless a permission preset is set) (#430, #475).
