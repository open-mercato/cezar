...
| `CEZ_AUTOPILOT=0` | Turn OFF **Autopilot** (Self-Heal Loop + Control Tower; spec `.ai/specs/2026-09-19-autopilot-heal-tower.md`). ON by default: the Tower nav shows live swarm telemetry (CPU/RSS/cost) across every project, governor caps live in `~/.cezar/autopilot.json` (all null until you set them — zero-config stays safe), and the built-in `cezar-self-heal` skill plus `/heal*` ledger let a task scout → spawn → verify → draft-PR without auto-merging. Only the exact value `0` turns it off; restart after changing it. |
