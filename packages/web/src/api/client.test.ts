...
describe('request shapes', () => {
  const cases: Array<{
    ...
    },
    {
      ...
      method: 'POST',
      // The server's schema defaults all three, but an absent `images`/`files` and `[]` are the
      // same request — send the shape the endpoint documents rather than half of it.
