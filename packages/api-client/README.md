...
- **`ApiError`** — HTTP failures carry their response status; a native Fetch `TypeError` raised
  before any response is normalized to status `0` with the original error as `cause`. Aborts and
  non-Fetch programming errors are preserved unchanged.
