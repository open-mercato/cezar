...
- This skill itself never claims, labels, or mutates tracker state — it is read-only (**get-issue** only). The autofix chain's claim is taken by the `cez-fix` step; the label and claim bullets above describe the chain contract this step's report feeds into, not actions this step performs.
