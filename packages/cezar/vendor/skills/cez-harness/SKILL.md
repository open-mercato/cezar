...
`om-implement-feature`, or `cez-setup-harness`.
...
- `prepare-review --config <path> --worktree <path> --kind <spec|diagnosis|implementation> --output <path>` — bind the exact subject, validation evidence, repository rules, and installed `cez-code-review` rubric into one review packet.
- `review --config <path> --profile <name> --worktree <path> --review-packet <path> --host-review <path> --output-dir <path>` — run every configured advisor while waiting for the concurrent fresh Claude pass, validate that pass, and aggregate all results from the same `cez-code-review` packet.
...
- Start the fresh Claude `cez-code-review` context and bound provider council concurrently, then require Claude's hash-matched artifact before emitting the final result. Never let the conductor's implementation context stand in for that pass.
- Give every configured advisor the complete installed `cez-code-review` rubric and the same resolved packet in a fresh invocation. Never expose reviewer outputs to peers before reconciliation.
