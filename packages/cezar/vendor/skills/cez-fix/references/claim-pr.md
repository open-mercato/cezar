...
- The chain driver (`om-auto-fix-issue`, via `cez-verify-in-repo`) already ran the three-signal in-progress check before this step started; cez-fix does not re-litigate ownership — it applies the idempotent claim on `{issueId}`.
