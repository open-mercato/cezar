...
- The claim-etiquette bullet's "release your own claim when done" is deliberately deferred here: cez-fix is a mid-chain step, so the lock it applies is released by `om-open-pr` (success) or an external janitor (failure) — never by this step. Details: `references/claim-pr.md`.
