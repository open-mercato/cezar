...
chain `cez-verify-in-repo` → `cez-root-cause` → `cez-fix` → `om-open-pr` →
...
descriptors shipped inside `cez-setup-pipeline`). A cherry-picked install
...
ROSTER="om-apply-upgrade-notes om-approve-merge-pr om-auto-continue-pr om-auto-continue-pr-loop om-auto-create-pr om-auto-create-pr-loop om-auto-fix-issue om-auto-fix-pr om-auto-implement-spec om-auto-manage-issues om-auto-qa-pr om-auto-review-pr om-auto-update-changelog om-auto-write-spec om-check-and-commit om-close-fixed-issues cez-code-review om-create-skill cez-fix om-fix-issue om-fix-issue-multi om-fix-issue-multi-optimized om-fix-issue-optimized om-followup-issue-from-pr cez-harness om-implement-feature om-implement-feature-multi om-implement-feature-multi-optimized om-implement-feature-optimized om-integration-tests om-merge-buddy om-open-pr om-prepare-issue om-prepare-test-env om-review-prs cez-root-cause cez-setup-harness cez-setup-pipeline cez-spec-writing cez-verify-in-repo"
missing=""
...
```bash
npx skills add <collection-source> --skill cez-fix --skill cez-root-cause
```
