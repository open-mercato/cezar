...
/**
 * The cez-* skills shipped inside the package (`vendor/skills/`, generated by
 * `scripts/vendor-skills.mjs` — spec 2026-07-24-vendored-cez-skills). This
 * file compiles to `dist/skills.js` and runs from `src/` under tsx; both sit
 * one level below the package root, so `../vendor/skills` resolves in dev,
 * from a git checkout, and from the published tarball alike.
 */
export function bundledSkillsDir(): string {
  return resolve(dirname(fileURLToPath(import.meta.url)), '..', 'vendor', 'skills');
}
...
export async function discoverSkills(
  ...
  const [lists, bundledSkills, gatedRepos, uiState] = await Promise.all([
    ...
    bundledDir === null ? Promise.resolve([]) : readMarkdownSkills(bundledDir, 'bundled'),
