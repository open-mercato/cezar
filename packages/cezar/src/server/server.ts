...
export function createApp(deps: ServerDeps) {
  ...
  const runsRoutes = new Hono<ProjectApiEnv>()
    ...
    .post('/runs/:id/continue', jsonZodValidator(continueSchema, { absent: ({}) }), async (c) => {
      ...
      // would cross the very billing boundary accounts exist to draw. Checked BEFORE the file
      // materialization below, so a rejected continue writes nothing to disk.
      if (parsed.data.agentProfile !== undefined) {
