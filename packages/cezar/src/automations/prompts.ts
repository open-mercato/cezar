 ...
 */
export const AUTOMATION_SCHEMA_REFERENCE = `An automation definition is one JSON object. Two kinds: a GitHub poll ("kind": "github") or a schedule ("kind": "schedule").
...
For a GitHub poll, name, events, intervalSeconds, filters and task are required; for a schedule, name, kind, schedule and task. No other top-level key is accepted. Schedules run in the cockpit's own time zone.
...
Always through the cockpit's own binary, node "$CEZ_BIN", because a cez on your PATH may be an older install without this command; every "cez automation …" below means node "$CEZ_BIN" automation …:
  ...
  cez automation check <id> [--execute]              GitHub poll: preview what the filter matches right now (launches nothing without --execute)
...
You turn the user's request into an automation on this cockpit: a bounded poll of this repository's pull requests or issues, or a schedule, that launches an ordinary cezar task for every match or occurrence. You do NOT do the recurring work yourself, and you do not add cron jobs, GitHub Actions, webhooks or polling scripts to the repository — the cockpit already polls and already keeps time.
