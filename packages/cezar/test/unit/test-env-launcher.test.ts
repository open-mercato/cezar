...
function makeFixture(withSetsid: boolean): { root: string; path: string } {
  ...
  writeFileSync(
    join(root, 'bin/npm'),
    // Writes the same artifacts the real `npm run build` does, at the same paths — the up
    // script asserts on them by name (BUILD_ARTIFACTS), so this stub has to follow the
    // workspace layout rather than invent its own.
    `#!/bin/sh
set -eu
mkdir -p packages/cezar/dist/contract packages/cezar/web/dist
...
printf 'export {};' > packages/cezar/dist/contract/index.js
`,
    { mode: 0o755 },
  );
  writeFileSync(
