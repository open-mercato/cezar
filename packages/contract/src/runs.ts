...
/**
 * `POST /runs/:id/continue`. An empty body preserves the current engine choices. A supplied
 * reasoning effort replaces the Codex selection; `''` explicitly returns to the native default.
 */
export const continueRunInputSchema = z.object({
  ...
  /** Agent account for the reopened session. Omitted keeps the run's current account. */
  agentProfile: z.string().max(64).optional(),
