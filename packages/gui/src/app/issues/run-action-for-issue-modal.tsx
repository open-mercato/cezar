 ...
 */
export function RunActionForIssueModal({ issueNumber, issueTitle, onClose }: RunActionForIssueModalProps) {
  ...
  return (
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="run-action-for-issue-title"
        className="w-full max-w-md rounded-lg border border-outline-variant bg-surface-container-high p-5 shadow-ambient"
      >
        <h2 id="run-action-for-issue-title" className="text-base font-semibold text-on-surface">
        ...
        <div className="mt-4 space-y-3">
          <label className="block">
            ...
            >
              ...
              {!actionsLoading && actions.length === 0 && <option value="">No enabled issue-target actions</option>}
              {actions.map((a) => (
