...
7. **`@open-mercato/cezar-api-client` is published from the start**, not once the server needs it at runtime. It is the deliverable other products install. Consequence, already implemented: the release pipeline stamps and publishes a three-package set in dependency order (api-client → cezar → alias), re-pinning intra-release ranges in whichever dependency section declares them.
