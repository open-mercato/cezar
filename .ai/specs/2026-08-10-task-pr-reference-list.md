...
A task's PR association is single-valued today: the newest declaration silently replaces the previous one (`packages/cezar/src/runs/store.ts:862` — `applyMarkerRefs` assigns `run.prNumber = refs.pr` and re-resolves the referenced URL against the new number). A follow-up turn that opens a second PR therefore erases the link to the first, and the original — usually still the main deliverable — becomes unreachable from the cockpit. This spec replaces the single slot with an **ordered, append-only list of PR references** (`prRefs`) on `RunRecord`, makes `prNumber` a **projection of that list** and keeps `pullRequestUrl` and `referencedPullRequestUrl` written by exactly the rules they follow today — so every existing reader, script and hand-edited `runs.json` keeps parsing, and a rollback still finds the fields it expects — and paints the extra references as additional chips beside the primary one. Exactly one scalar changes behaviour (`prNumber`, which starts tracking a created PR it ignored before); the other two are preserved rather than derived, and the only read that moves is the chip's fallback to `referencedPullRequestUrl`, which becomes a read of the list. A second phase hydrates PR state lazily so a closed-and-replaced PR is dimmed and can hand the primary slot to its successor — the one case where today's "last one wins" is the right answer.
...
Promote the display tier's PR association from a scalar to a **list**, and make `prNumber` a derived view of that list:
...
- **Every writer that records a PR association** goes through one private store helper instead of assigning a scalar. That is not three call sites but **six**, and enumerating them is the difference between a projection and a field that drifts. They divide into two groups, and the distinction matters because it is the whole of this design's behavioural change:
  - **Four already assign `run.prNumber`** and simply stop: `applyMarkerRefs` (`store.ts:862`), the step-0 regex extraction at run creation and on resume (`workflows/run.ts:769`, `:1835`) and the fire-and-forget LLM namer (`workflows/run.ts:3179`).
  - **Two assign `pullRequestUrl` today and never touch `prNumber`**: the created-PR janitor (`store.ts:758`) and the draft-PR route (`server.ts:4127`). They gain a `recordPrRef(..., 'created')` call *beside* their existing write. Because `created` is the top provenance rank, this is what makes `prNumber` start naming a created PR where today it keeps whatever the task text happened to say — a real, intended change to a persisted value, declared under **Data Model** and in `BACKWARD_COMPATIBILITY.md` §3 rather than glossed as "unchanged".

  A writer that keeps assigning `prNumber` directly is a writer that desynchronises the projection the first time it fires after an append.
...
- **The other two scalars are preserved, not projected — and that is deliberate.** `pullRequestUrl` keeps first-wins semantics because the action gates read it directly (see **Architecture → The tiers stay as they are**), and `referencedPullRequestUrl` keeps being re-resolved from `markerRefs.pr`, the *most recently declared* PR, because that is precisely what its name means and what a rolled-back cezar needs to find there. Neither becomes a function of the list. What changes is that the **cockpit stops reading `referencedPullRequestUrl` for the chip** and reads `taskPrRefs` instead, so the rendered primary follows the list's ordering rule rather than the last declaration. The two can therefore disagree on a multi-declaration task — `prNumber` and the chip say `#A` (earliest `marker`) while `referencedPullRequestUrl` still points at `#B` — and that is the bounded, intended cost of keeping rollback free: an older cezar reading the same file renders `#B`, which is exactly today's behaviour.
...
- **`prNumber` is a projection — of the *offline* primary.** Every writer goes through the store helper, and the helper recomputes `prNumber` from the Phase 1 primary; no code path assigns the field directly (see the three `workflows/run.ts` writers in the Architecture table — un-migrated, they are exactly how a "projection" silently becomes a stale field again). Its type and presence rules are unchanged.
- **`prNumber`'s *value* does change for one common shape of run — declared, not incidental.** Today the created tier never writes `prNumber`: the janitor and the draft-PR route set `pullRequestUrl` only (`store.ts:758`, `server.ts:4127`). Once they record a `created` entry — the top provenance rank — the created PR takes the primary slot and therefore the field. So a run started from the task text *"port the fix from pr 441"* that then opens PR #900 reports `prNumber === 441` today and `prNumber === 900` after Phase 1. This is the more truthful answer, and it is the one the plan's step 4 regression test pins — but it is a value change to a §3-protected field, so it is written down under **Compatibility with `runs.json`** and in the CHANGELOG rather than asserted away. Who notices: the `CEZ:TITLE=` prefix (`workflows/run.ts:3233` reads `run.prNumber`), the run DTO, and scripts reading `runs.json`. Who does not: the rendered chip, which already prefers the created PR's URL-derived number (`tasks-table.ts:174-175`) and shows `#900` today.
...
- **One existing field changes its values, and §3 requires that to be stated.** `prNumber` begins tracking a PR the run *created* where it previously kept a number scraped from the task text (see **Data Model**). No shape, type or presence rule moves, and nothing becomes unparseable — but the same history can now yield a different number, which is the same class of change as the `diffStat.repointed` value shift §3 already documents ("what *did* change for those runs is the **values**"). Historical records are **not** backfilled: a finished run keeps the `prNumber` it was written with, and only a run that appends a new reference recomputes. The §3 bullet and the CHANGELOG entry in Phase 1 step 10 both carry this.
- **`referencedPullRequestUrl` is untouched by this spec** — still re-resolved from the latest declaration. The cockpit stops *reading* it for the chip (it reads `taskPrRefs`), so on a multi-declaration task the persisted field and the rendered chip can name different PRs. That is deliberate: it is what lets a downgraded cezar keep rendering today's answer from the same file.
...
| Task text names `#441`, the run then opens `#900` | `prNumber` moves from `441` to `900` — the created entry outranks the derived one. **This is the one value change to a persisted field** (see **Data Model**); `441` survives as a non-primary `derived` entry instead of being the only number on record. The chip is unaffected: it already shows `#900` today. |
| Both `#A` and `#B` declared; a reader compares `prNumber` with `referencedPullRequestUrl` | They disagree by design — `prNumber` is `A` (earliest `marker`, the list's primary) and `referencedPullRequestUrl` still resolves to `B` (the latest declaration, rule unchanged). The cockpit reads the list, so the chip agrees with `prNumber`; a downgraded cezar reads the old field and renders `B`, exactly as today. |
...
- **Blast radius: the display tier of the task record.** Every action gate (`Draft PR`, `Create PR → View PR`, push) keeps reading `pullRequestUrl` directly and is untouched. The riskiest single line is the `prNumber` write becoming a projection — and the risk is not the line, it is the **five other places that record an association** (`store.ts:758`, `server.ts:4127`, `workflows/run.ts:769`, `:1835`, `:3179`), three of which write `prNumber` today and two of which write only `pullRequestUrl`. Migrating them all is a hard requirement of Phase 1, not a follow-up; one left behind reintroduces a `prNumber` that names a PR absent from `prRefs`, and nothing ever repairs it. Covered by the existing marker/namer precedence tests plus new ones asserting the projection holds after each writer fires.
- **The declared value change to `prNumber` (§3).** Enrolling the created tier is exactly what makes `prNumber` start naming a created PR, so the fix for the drift risk above *is* the source of this one; they cannot be separated. Bounded to runs whose task text named a different PR than the run opened, non-destructive, and no historical record is rewritten. Documented under **Data Model** and **Compatibility with `runs.json`**, with a CHANGELOG line — the same treatment §3 gave the `diffStat.repointed` value shift.
...
2. **Wire `applyMarkerRefs`.** Replace `run.prNumber = refs.pr` with an `appendPrRef(..., 'marker')` call; `markerRefs.pr` and the `referencedPullRequestUrl` re-resolution stay exactly as they are — `referencedPullRequestUrl` is **not** made a projection (see **Proposed Solution**), it keeps naming the latest declaration so a rollback finds today's answer. Extend `store.test.ts`: two declarations keep both numbers, the first stays primary, `prNumber` reports the primary, a third declaration of the first number changes nothing, and **a declaration still beats a `derived` number from the task text** (today's `store.ts:848-852` rule, now enforced by rank). One test pins the intended divergence explicitly: after `CEZ:PR=A` then `CEZ:PR=B`, assert `prNumber === A` *and* `referencedPullRequestUrl` still resolving to `B` — so the split is a fixture, not a surprise found later.
3. **Wire the created tier.** The janitor (`store.ts:758`) and the draft-PR route (`packages/cezar/src/server/server.ts:4118-4127`, beside its existing `pullRequestUrl: outcome.url` write — note that `server/pr.ts` is only a re-export barrel and `createDraftPr` in `forge/github.ts:1401` persists nothing) append a `created` entry. **Neither writes `prNumber` today**, so this step is the one that changes a persisted value: it is where `prNumber` starts naming the created PR. Tests: a created PR is primary even when a marker declared a different PR first; and the value change itself is pinned — a run whose task text named `#441` and which then opens `#900` moves `prNumber` from `441` to `900`, with `441` retained as a non-primary `derived` entry rather than lost.
...
6. **Selector.** Add `taskPrRefs(run, repoBase?)` to `packages/web/src/lib/tasks-table.ts`, re-express `taskPrUrl`/`taskReference` on it, and prove in `tasks-table.test.ts` that every existing case (created wins, #526 issue-subject guard, legacy record, no-PR run) returns byte-identical results. This is where the cockpit stops falling through to `referencedPullRequestUrl` (`tasks-table.ts:139`) and reads the list's primary instead — the change that keeps the chip agreeing with `prNumber` on a multi-declaration task. Test it directly: `CEZ:PR=A` then `CEZ:PR=B` with no created PR renders `#A`, while the record's `referencedPullRequestUrl` still points at `#B`. `derived` entries are excluded from the rendered list unless they are the only one.
...
10. **Docs.** `BACKWARD_COMPATIBILITY.md` §3 bullets for (a) `prRefs` including the downgrade note and (b) **the `prNumber` value change** — that the field now tracks a created PR where it previously kept a number scraped from the task text, with no backfill of historical records; the **§8 entry** recording the narrowing of `CEZ:PR=`'s display effect; the matching amendment to the handoff instruction text at `packages/cezar/src/handoff.ts:152`; a CHANGELOG entry naming the `prNumber` shift as user-visible (task-title prefixes move with it); and the `AGENTS.md` runs-store row if the invariant list there needs the append-only rule.
