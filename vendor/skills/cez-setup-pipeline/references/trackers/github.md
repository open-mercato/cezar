...
Create every label from the config's taxonomy that does not exist yet (used by `cez-setup-pipeline`; skip ones that already exist per **list-labels**):
```bash
